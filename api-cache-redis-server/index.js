...
app.get("/unsplash/*", (req, res) => {
  ...
  let url = `https://api.unsplash.com${inputUrl.slice(9)}`;
  let redisKey = `unsplash:${inputUrl.slice(9)}`;

  if (inputUrl === "/unsplash/") {
    return res.json({ error: "No query provided" });
  }
  getUnsplashfromCache();
  //getUnsplash();

  async function getUnsplashfromCache() {
    try {
  ...
  async function getUnsplash() {
    try {
      ...
      client.set(redisKey, JSON.stringify({ source: "Redis Cache", ...responseJSON }));
